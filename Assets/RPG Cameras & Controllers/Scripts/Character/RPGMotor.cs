...
namespace JohnStairs.RCC.Character {
    public abstract class RPGMotor : MonoBehaviour {
        ...
        /// <summary>
        /// True if the character was grounded during the last FixedUpdate
        /// </summary>
        protected bool _groundedLastFixedUpdate;
        /// <summary>
        ...
        protected virtual void Start() {
            ...
            _colliderBottomOffset = GetColliderPosition(ColliderSide.Bottom).y - transform.position.y;
        }

        protected virtual void FixedUpdate() {
            if (!_swimming && !_groundedLastFixedUpdate && _grounded) {
                // Ensure that the character's X and Z axes rotations are reset after swimming (and flying) to prevent tilted ground movement
                ResetXandZrotations();
            }

            _groundedLastFixedUpdate = _grounded;
        ...
        public virtual void StartMotor() {
            ...
            if (swamLastFrame && !_swimming) {
                // Character stopped swimming this frame
                forceGroundedCheck = true;
            }

            if (_movementDirection.y < 0 || forceGroundedCheck) {
        ...
        protected virtual void SetAnimatorParameters() {
            ...
            _animator.SetBool("Sliding", _slidingBuffer < -SlidingTimeout); // buffer full due to many consecutive sliding events => fire animation
            _animator.SetBool("Swimming", _swimming);
        }

        /// <summary>
        /// Immediately translates the character. Should be called from within FixedUpdate so that the Character Controller component does not 
        ...
        protected virtual void ApplySliding() {
            ...
            foreach (RaycastHit hit in hits) {
                ...
                if (Mathf.Round(slope) > _characterController.slopeLimit) {
                    slide = true;
                    // Add sliding force
                    moveDirection += slidingDirection * slope * 0.2f;
                } else {
                    slide = false;
                    break;
                }
        ...
        protected virtual void PreventSwimmingAboveSurface() {
            ...
            if (_movementDirection.y * Time.deltaTime + _swimmingStartHeight + 0.5f > _currentWaterHeight) {
        ...
        /// <summary>
        /// Gets a position on a collider
        /// </summary>
        /// <param name="side">Which side to get</param>
        /// <returns>The position of the given side in world coordinates</returns>
        protected virtual Vector3 GetColliderPosition(ColliderSide side) {
            return transform.TransformPoint(_characterController.center) + (side == ColliderSide.Top ? Vector3.up : Vector3.down) * _characterController.height * 0.5f;
        }
        ...
        protected virtual void UpdateColliderBottom() {
            _colliderBottom.center = GetColliderPosition(ColliderSide.Bottom) + Vector3.up * _characterController.radius;
        ...
        protected virtual void OnDrawGizmosSelected() {
            ...
            Gizmos.color = blue;
            Gizmos.DrawCube(transform.TransformPoint(_characterController.center) + Vector3.up * (SwimmingStartHeight - _characterController.height * 0.5f), new Vector3(0.7f, 0.01f, 0.7f));
        }
    }
}
