...
namespace JohnStairs.RCC.Character {
    public abstract class RPGCamera : MonoBehaviour {
        ...
        protected virtual void Start() {
            ...
            HandleCursor();
            if (!UseNewInputSystem && CursorBehaviorOrbiting == CursorBehavior.Stay) {
                Debug.LogWarning("Cursor Behavior While Orbiting is set to \"Stay\" but is only supported with Unity's new input system! Choose another behavior or set variable \"UseNewInputSystem\" to true");
            }
        ...
        protected virtual void SetUpInputActionCallbacks() {
            ...
            _inputActions.Character.AllowOrbiting.started += context => {
                ...
                _orbitingStartedOverGUI = _pointerInfo?.IsPointerOverGUI() ?? false;
            };
        ...
        protected virtual void HandleCursor() {
            if ((_pointerInfo?.IsPointerOverGUI() ?? false)
                ...
                // Prevent also orbiting which started outside the GUI but now entered it
                _orbitingStartedOverGUI = true;
                // Reset the stored cursor position
                _tempCursorPosition = Vector2.zero;

                if (CursorBehaviorOrbiting == CursorBehavior.LockInCenter) {
                    // Unlock the cursor again
                    Cursor.lockState = CursorLockMode.None;
                }

                if (HideCursor == CursorHiding.WhenOrbiting) {
            ...
            bool orbitingStart = _inputAllowOrbitingStart || _inputAllowOrbitingWithCharRotationStart;
            bool orbiting = _inputAllowOrbiting || _inputAllowOrbitingWithCharRotation;
            bool orbitingStop = _inputAllowOrbitingStop || _inputAllowOrbitingWithCharRotationStop;
            ...
            if (CursorBehaviorOrbiting == CursorBehavior.Stay) {
                if (orbitingStart) {
                    ...
                    // Workaround for a bug. Fix is pending for 2019.4
